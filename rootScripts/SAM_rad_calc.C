...
//use parserHallrad format to loop over files instead of doing it here
  ...
  int nEvents = 450e6;//9e6 for regular runs, 54e6 for benchmark runs;
  if (units!=ben){ //case = doing parameter space searches
    ...
    //file_in = TFile::Open(Form("output/list_%s_%i%s_hallRad.root",modifier.c_str(),number,units.c_str()));
    file_in = TFile::Open(Form("output/list_%s_hallRad.root",modifier.c_str()));
  }
  else{ //case = benchmark
    //nEvents=450e6;
...
*/
  // - Old E, old target position for prexII: Absolute per event: Neil in LHRS 1001 = 1.0121e-05(1.28749e-07), Flux of neutrons to 1006, per million events (E>25.12 MeV) = 1.27963e-05(1.6863e-07), Energy in 3201 = 0.0113008(5.94492e-06)
  // - New E, new target position for prexII: Absolute per event: Neil in LHRS 1001 = 1.09249e-05(1.38434e-07), Flux of neutrons to 1006, per million events (E>25.12 MeV) = 1.47289e-05(1.80917e-07), Energy in 3201 = 0.0117311(5.97722e-06)
  Double_t bench_NEIL_1001 = 1.09249e-05;
  Double_t bench_NEIL_1001_error = 1.38434e-07;  // error propagation = sigma = value after modification * relative error of subparts added in quadratures (sigma/previous value)
  Double_t bench_Flux_1006 = 1.47289e-05;
  Double_t bench_Flux_1006_error = 1.80917e-07 ;//54M is the number of beam events per run that the averages normalize to
  Double_t bench_Energy_3201 = 0.0117311;
  Double_t bench_Energy_3201_error = 5.97722e-06;
...
//}
  ...
  file_out.open(Form("output/SAM_%s_overview.csv",modifier.c_str()),std::ofstream::out | std::ofstream::app);
  NEIL_1001_out.open(Form("output/SAM_%s_NEIL_1001.csv",modifier.c_str()),std::ofstream::out | std::ofstream::app);
  Flux_1006_out.open(Form("output/SAM_%s_Flux_1006.csv",modifier.c_str()),std::ofstream::out | std::ofstream::app);
  Energy_3201_out.open(Form("output/SAM_%s_Energy_3201.csv",modifier.c_str()),std::ofstream::out | std::ofstream::app);
  ...
  file_out<<"Absolute per event: Neil in LHRS 1001 = "<<NEIL_1001<<"("<<NEIL_1001_error<<"), Flux of neutrons to 1006, per million events (E>25.12 MeV) = "<<Flux_1006<<"("<<Flux_1006_error<<"), Energy in 3201 = "<<Energy_3201<<"("<<Energy_3201_error<<")"<<std::endl;
  file_out<<"Relative per event: Neil in LHRS 1001 = "<<NEIL_1001_ratio<<"("<<NEIL_1001_ratio_error<<"), Flux of neutrons to 1006, per million events (E>25.12 MeV) = "<<Flux_1006_ratio<<"("<<Flux_1006_ratio_error<<"), Energy 3201 = "<<Energy_3201_ratio<<"("<<Energy_3201_ratio_error<<")"<<std::endl;
