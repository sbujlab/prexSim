...
#FIXME Update these
    ...
    identifier = "SAMs_cyl_canthick"#raw_input("Please enter the identifier: ")
    canthick = raw_input("Please enter the canthick in um (integers up to 1651; .1651cm=1.651mm=1651um - or higher if needed): ")

    f = open('../geometry/SAMs_cyl_canthick.xml', 'w')
    fileout = '    <constant name="sam_can_wall_thickness" value="' + canthick + '.0/10000"/>\n    <constant name="sam_can_face_thickness" value="' + canthick + '.0/10000"/>'
    f.write(fileout)
    f.close()

    #sourceDir = "/work/halla/parity/disk1/ciprian/prexSim"
    sourceDir = "/work/halla/parity/disk1/moller12gev/cameronc/prexSim"
    outputDir = "/lustre/expphy/volatile/halla/parity/cameronc/prexSim/output/SAM_canthick_tests"
    nrEv   = 900000 #900000
    nrStart= 0
    nrStop = 10 #60
...
#</FIXME>
    ...
    jobName=configuration + '_' + identifier + '_' + canthick + 'um' + '_%03dkEv'%(nrEv/1000)
